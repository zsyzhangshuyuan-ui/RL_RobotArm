    ...
    public class SplineControlledParticleSystem : MonoBehaviour
    {
        ...
        void LateUpdate()
        {
            ...
            for (int i = 0; i < aliveParticlesCount; i++)
            {
                ...
                Vector3 offshootDirection =
                    Quaternion.AngleAxis(1080 * seedAsPercent, -_spline.GetDirection(travelPercentage, false)) *
                    _spline.Up(travelPercentage);
                Vector3 previousOffshootDirection =
                    Quaternion.AngleAxis(1080 * seedAsPercent,
                        -_spline.GetDirection(travelPercentage - _previousDiff, false)) *
                ...
                if (travelPercentage - .01f >= 0)
                    lastPosition = _spline.GetPosition(travelPercentage - _previousDiff, false);
                ...
                switch (_particleSystem.main.simulationSpace)
                {
                    case ParticleSystemSimulationSpace.Local:
                        ...
                        currentPosition =
                            _particleSystem.transform.InverseTransformPoint(position + offshootDirection * offset);
                        previousPosition =
                            _particleSystem.transform.InverseTransformPoint(lastPosition +
                                                                            previousOffshootDirection * previousOffset);
                        break;
